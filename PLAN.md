...
| 50 | Ressorts Sport + Kultur | **Neu (2026-07-13):** Zwei zusätzliche Topics ergänzt, angehängt in dieser Reihenfolge (`…wissenschaft, sport, kultur`). **Kultur statt Musik:** „Musik" allein wäre zu eng neben den breiten Bestands-Ressorts — „Kultur" (Musik, Film, Theater, Kunst, Literatur, Kulturbetrieb) hält die Breite konsistent und absorbiert Musik. Grenzregeln im Prompt (`claude.ts`) + Glossar ([CONTEXT.md](CONTEXT.md)): Sport = Sport als Gegenstand, aber Unglück/Kriminalität/Politik dominiert → altes Ressort (tödlicher Rennunfall bleibt „gesellschaft"); Kultur = reines Kulturereignis/-betrieb, Debatte drumherum bleibt „gesellschaft", Medienethik-Verfahren bleibt „gesellschaft". Enum an allen 6 Stellen erweitert (content.config.ts, articles.ts TOPICS+LABELS, types.ts, claude.ts Prompt-Schema+Zuordnungsregeln+Validierungs-Set); `/themen/*` + validate.ts sind topic-getrieben und ziehen automatisch mit. **Bestand umgetaggt → kultur:** Holzinger-Bodensee-Performance, Sandbichler-KHM-Kündigung (klare Kulturbetriebs-Fälle); Presserat-Falter und Jan-Völker-Interview bleiben „gesellschaft". **Zwei bewusst verschobene Restpunkte (Go-Live-TODOs):** (a) Ressort-Symbolbild-Fallback (`commons.ts` RESSORT_QUERIES) bewusst NICHT für Sport/Kultur gesetzt — Subgenres zu divers (Beachvolleyball≠Fußballstadion, Techno≠Theaterbühne), Event-Match trägt, sonst Text-Teaser; genauere Bildlogik folgt separat. (b) Scoring (`score.ts`) unangetastet: `LOW_VALUE_KEYWORDS` (−0,3, u. a. fußball/ski/tennis/olympia/match) und `EVENT_PR_KEYWORDS` (−0,15, konzert/festival/premiere) unterdrücken Sport/Kultur weiter. **Revidiert (2026-07-13, nach Verifikationslauf mit 0 Sport/Kultur):** neues additives `MAJOR_EVENT_KEYWORDS` (+0,2; Championship-/Titel-/Weltrekord-Ebene: weltmeister\*, europameister\*, weltcup, weltrekord\*, olympi\*, medaille\*, meistertitel, grand slam + große Kulturpreise/Flaggschiff-Festivals: nobelpreis\*, welturaufführung, berlinale, viennale, oscar(s), grammy(s), salzburger/bregenzer festspiele, goldene(r) löwe/palme). Malus bleibt bewusst bestehen — der Bonus reißt die Triage-Schwelle erst ZUSAMMEN mit ≥2 Portalen + Frische; Routine ohne diese Deckung bleibt draußen. Replay-Kalibrierung: WM-Finale/Festspiele/Nobelpreis (breit+frisch) → ai_triage 0,70–0,85; Routine-Ligaspiel selbst über 3 Portale → nur 0,50 (monitor); Einzelkonzert/Einzelspiel 1 Portal → ignore. **Quellen (2026-07-13):** Verifikationslauf zeigte, dass gar keine echten Sport/Kultur-News im Pool waren — Ursache: Sport-Feeds `orf_sport`/`derstandard_sport` standen auf `enabled: false`, Kultur-Feeds fehlten ganz. In `data/ai-news/sources.yaml` Sport-Feeds aktiviert (weight 0,2 als Filter — Großevents heben den Cluster-Max-Weight über die parallel laufenden General-Feeds) und Kultur-Feeds ergänzt (`derstandard_kultur`, `diepresse_kultur`, `kurier_kultur`). Dry-Run-Verifikation an echtem Feed: Wimbledon-Finale (Sinner) → ai_triage 0,95, Routine-Etappen/-Matches/-Einzelkonzerte → ignore; von 216 Clustern nur 34 ai_triage (kein Flood). Threshold/Malus bewusst NICHT gesenkt — das hätte nur Boulevard (Promi-Lookalike, Gewinnspiel-PR) durchgelassen, nicht echten Sport. **Nachtrag (2026-07-13, 2. Gate):** Erster Realbetrieb zeigte, dass das geöffnete Score-Gate allein nicht reicht — Sinners Wimbledon-Titel erreichte zwar ai_triage 0,95, wurde aber vom Haiku-Triage-Prompt (`claude.ts`) mit „Reiner Sport/Promi/Lifestyle → ignore" wieder gekippt (kein Österreich-Bezug; Zverev/Sinner ausländisch). Non-draft-Triage wird NICHT rückgestellt (`run.ts` ~Zeile 380 `continue`, kein requeue) → die Story verschwand spurlos statt im Backlog zu landen. Zwei Gates, die sich widersprachen: Score-Gate offen, Editorial-Gate zu. Fix: Triage-Prompt an E50 angeglichen — Routine-Sport bleibt `ignore`, aber Sport-Titel-/Championship-Ebene (WM/EM/Olympia/Grand-Slam-Titel/Weltrekord) und große Kulturereignisse (Nobelpreis, Berlinale/Viennale, Festspiele, Oscar/Grammy) rechtfertigen `draft_article` auch OHNE Österreich-Bezug, sofern breit berichtet (≥2 Portale); Newsworthiness solcher Events mindestens 3. Der Österreicher-Heimsieg (Mühlberger, Tour of Austria 0,85) kam durch, weil er nationale Relevanz hatte — das ausländische Grand-Slam-Finale fiel bis dato durchs Editorial-Gate. |
| 51 | Aufmacher in Ressorts + Themenseiten | **Neu (2026-07-13), revidiert E38/E42/E46 teilweise:** Ressort-Sektionen der Startseite und die Themen-Seiten (`/themen/*`) waren rein chronologisch — populäre Stories konnten dort unter jüngeren, schwächeren Meldungen verschwinden. Jetzt bekommt **jede Ressort-Sektion und jede Themen-Seite genau einen Aufmacher oben**, der Rest bleibt chronologisch wie bisher. **Auswahl:** Aufmacher = höchster `relevanceScore` (`byRelevance`, E38/E46 — Newsworthiness + Quellenqualität + Resonanz-Echo + Frische-Decay) innerhalb des Themas; keine echten Zugriffszahlen zur Build-Zeit (statische Site, GA4 optional). Rest = `byNewest`, Aufmacher aus der Liste entfernt (keine Dopplung **innerhalb** einer Sektion). **Dopplung mit dem globalen Top-Bereich der Startseite (Lead + „Weitere Nachrichten", E38) ist erlaubt** — konsistent mit E42 (Top = Relevanz, Ressort = Chronologie); ein thematischer Top-Artikel darf oben und im Ressort erscheinen. **Globaler Top-Bereich bleibt unverändert** (eigenes 2-Spalten-Markup in `index.astro`). **Bauteil:** neues `src/components/Aufmacher.astro` (voll-breite Lead-Optik: Bild links / Text rechts, Serif-Titel, Standfirst, Meter-Metriken Primärquellen/Einseitigkeit/Belegt-Quote, Badges Korrigiert + „Breites Echo"), geteilt von Ressort-Sektion und Themen-Seite. Eyebrow zeigt Land + Datum + Badges, **kein** Topic (Kontext ist bereits thema-scoped); `eager`-Prop nur für den Themen-Seiten-Kopf (above the fold). Ressort-Track/Pager rendern nur noch, wenn nach Abzug des Aufmachers Rest übrig ist (Ein-Artikel-Ressort → nur Aufmacher, kein leerer Track). RSS + „Weiterlesen" bleiben chronologisch (unberührt). |
| 52 | Geografische Nähe im Relevanz-Ranking | **Neu (2026-07-13), revidiert E38/E46:** `relevanceScore` (`articles.ts`) gewichtete Geografie **gar nicht** — `country` wurde angezeigt (Eyebrow), floss aber nie in die Reihung ein. Folge: ein reines Auslandsereignis konnte allein über hohe `newsworthiness` (40 % der Quality) + Frische zum globalen Aufmacher werden, ohne Österreich-Bezug (Beispiel: Bangkok-Bar-Brand, `country: int`, `newsworthiness: 5`, am selben Tag geupdatet → Lead der Startseite). **Fix:** multiplikativer Nähe-Faktor `PROXIMITY_SCORE` auf **nur** `base` (Quality × Decay), nicht auf das Resonanz-Echo — so kann eine breite Medienwelle (E46) eine Auslandsstory weiterhin heben, aber Newsworthiness allein reicht nicht mehr. Gewichte (kalibriert „mittel"): `at 1.0 · de 0.85 · eu 0.75 · int 0.5` → ein reines Auslandsereignis braucht ~doppelte Basis-Qualität, um eine solide AT-Story zu schlagen. Verifiziert am Build: Lead wechselte von Bangkok (int) zu einer AT-Politik-Story, Bangkok fiel aus den Top-5. **Bewusst NICHT geändert:** die Pipeline-Triage, die dem Ereignis `newsworthiness: 5` gab — das ist ein separater Defekt (Editorial-Gate, vgl. E50), hier nur die Ausspielung korrigiert. `country` bleibt grob (at/de/eu/int); eine echte AT-Betroffenheit einer int-Story ist damit nicht auflösbar — akzeptierter Trade-off gegenüber Pipeline-Umbau. |
